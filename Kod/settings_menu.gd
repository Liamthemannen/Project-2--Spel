...
func _on_volume_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_db(0, value) # Ändrar volym



func _on_mute_knapp_toggled(toggled_on: bool) -> void:
	AudioServer.set_bus_mute(0,toggled_on) # Mutar spelet

	# Sätter på fullscreen
...
# Döljer nuvarande settings menyn och öppnar paus meny
